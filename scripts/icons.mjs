...
// brand/uwulock-taskbar-icon.svg, upright, on a transparent background. At 16 and 24 px the ICO
// uses brand/uwulock-taskbar-icon-small.svg instead, with thicker outlines and a flat window,
// which otherwise turn to mush at that size. The macOS icon.icns and the Square*/StoreLogo tiles
// come from the app icon, brand/uwulock-app-icon.svg, like the website and GitHub.
...
function render(svg) {
  const out = mkdtempSync(join(tmpdir(), 'uwulock-icons-'));
...
const app = render('uwulock-app-icon.svg');
const large = render('uwulock-taskbar-icon.svg');
const small = render('uwulock-taskbar-icon-small.svg');
try {
