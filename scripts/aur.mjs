...
// --out it writes into target/aur/uwulock-bin. `pnpm release` calls it after
...
const REPOSITORY = 'MinifyX/UwULock-Client';
const DEBS = { x86_64: 'UwULock-linux-x64.deb', aarch64: 'UwULock-linux-arm64.deb' };
...
export function aurFiles({ version, sums }) {
  ...
  const template = readFileSync(join(root, 'packaging/aur/uwulock-bin/PKGBUILD.in'), 'utf8')
    .split('\r\n')
    .join('\n');
  ...
  const name = 'uwulock-bin';
  const lines = [
    `pkgbase = ${name}`,
    '\tpkgdesc = Password manager for Vaultwarden and Bitwarden',
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  const out = option('--out') ?? join(root, 'target', 'aur', 'uwulock-bin');
  ...
  console.log(`✧ PKGBUILD and .SRCINFO for uwulock-bin ${pkgver(version)} in ${out}`);
}
