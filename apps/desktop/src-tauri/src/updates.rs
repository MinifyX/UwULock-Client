...
//!
//! UwULock looks for a new version shortly after starting and every six hours,
//! downloads the signed setup (`UwULock-Setup-<version>.exe` on Windows) quietly
//! and tells the page. "Restart now" hands over to that setup in `--update`
//! mode; otherwise the update is applied the next time UwULock starts.
//!
...
const FEED: &str = "https://raw.githubusercontent.com/MinifyX/UwULock-Client/updates";
...
struct ReadyUpdate {
    version: String,
    /// Release notes; JSON with `de` and `en` when written for UwULock.
    notes: Option<String>,
...
fn setup_record() -> Option<PathBuf> {
    ...
    Some(config.join("app.uwulock.setup").join("install.json"))
}

/// Whether this copy of UwULock is one the setup put there, and so one the
...
#[cfg(target_os = "linux")]
const PACKAGE: &str = "uwulock";

/// How this copy of UwULock was installed, as far as updating it goes.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[cfg_attr(not(target_os = "linux"), allow(dead_code))]
enum Install {
    /// By UwULock's own setup, which the updater runs again.
    Setup,
...
/// Asked once; the answer doesn't change while UwULock runs.
...
/// Whether dpkg installed `exe` as part of UwULock's package.
#[cfg(target_os = "linux")]
...
/// Whether rpm installed `exe` as part of UwULock's package.
#[cfg(target_os = "linux")]
...
/// A waiting update, if it is where UwULock put it and still carries a valid
...
/// (`UwULock-windows-x64-setup.exe`, `UwULock-update-macos-universal`, …) but
...
fn setup_name(kind: Install, version: &str) -> String {
    let package = |ext: &str| format!("UwULock-{version}-linux-{}.{ext}", std::env::consts::ARCH);
    match kind {
    ...
    match (std::env::consts::OS, std::env::consts::ARCH) {
        ("macos", "aarch64") => format!("UwULock-Setup-{version}-macos-arm64-update"),
        ("macos", _) => format!("UwULock-Setup-{version}-macos-x64-update"),
        ("linux", "aarch64") => format!("UwULock-Setup-{version}-linux-arm64.AppImage"),
        ("linux", _) => format!("UwULock-Setup-{version}-linux-x64.AppImage"),
        (_, "aarch64") => format!("UwULock-Setup-{version}-windows-arm64.exe"),
        _ => format!("UwULock-Setup-{version}.exe"),
    }
}
...
/// Whether another UwULock window is running from the same program file. Its
...
fn hand_over(pending: Pending, relaunch: bool) -> Result<(), String> {
    ...
    #[cfg(unix)]
    {
        ...
        // Its own process group, so it lives on when UwULock quits.
...
fn install_package(pending: Pending, kind: Install) -> Result<(), String> {
    ...
    let (ext, command): (&str, &[&str]) = match kind {
        ...
        // rpm sorts `0.2.0-beta.1` after `0.2.0`; UwULock has already checked
    ...
    {
        use std::os::unix::process::CommandExt;
        // Its own process group, so it lives on when this UwULock quits.
        again.process_group(0);
    }
    again
        .spawn()
        .map(|_| ())
        .map_err(|e| format!("The update is installed, but UwULock couldn't start again: {e}"))
}
...
/// one. Returns true when UwULock must quit right away because the setup takes
/// over.
pub fn apply_pending_on_start(app: &AppHandle) -> bool {
    ...
    match open_pending(app, kind) {
        Some(pending) if is_newer(app, &pending.update.version) => {
            ...
            // prompt just for starting UwULock.
...
pub async fn install_now(app: &AppHandle) -> Result<(), String> {
    ...
    let kind = install_kind().ok_or("This copy of UwULock doesn't update itself.")?;
    if other_instances_running() {
        return Err(
            "Another UwULock window is still open. Close it first, so its connections aren't cut off."
                .into(),
        );
    }
...
/// Checks in the background for as long as UwULock runs.
...
mod tests {
    ...
    fn garbage_never_verifies() {
        assert!(!verify_with("", b"setup", "", "UwULock-Setup-1.0.0.exe"));
        assert!(!verify_with(
            "bm90IGEga2V5",
            b"setup",
            "bm90IGEgc2lnbmF0dXJl",
            "UwULock-Setup-1.0.0.exe"
        ));
    }
    ...
    fn the_signature_must_name_the_setup_the_feed_promised() {
        let comment = "timestamp:1789000000\tfile:UwULock-Setup-0.1.0-beta.2.exe";
        assert!(signs_file(comment, "UwULock-Setup-0.1.0-beta.2.exe"));
        // An older, validly signed setup offered as a newer version.
        assert!(!signs_file(comment, "UwULock-Setup-0.2.0.exe"));
        assert!(!signs_file(
            "timestamp:1789000000",
            "UwULock-Setup-0.2.0.exe"
        ));
    }

    #[test]
    fn only_the_file_uwulock_names_counts_as_a_setup() {
        let dir = Path::new("updates");
        let file = setup_file(dir, Install::Setup, "0.1.0-beta.2");
        let name = file.file_name().unwrap().to_string_lossy().into_owned();
        assert!(name.starts_with("UwULock-Setup-0.1.0-beta.2"), "{name}");
        #[cfg(all(windows, target_arch = "x86_64"))]
        assert_eq!(name, "UwULock-Setup-0.1.0-beta.2.exe");
        #[cfg(all(windows, target_arch = "aarch64"))]
        assert_eq!(name, "UwULock-Setup-0.1.0-beta.2-windows-arm64.exe");
        #[cfg(target_os = "linux")]
    ...
    fn packages_are_signed_under_their_own_versioned_names() {
        ...
        assert_eq!(
            ...
            format!("UwULock-0.2.0-linux-{arch}.deb")
        );
        assert_eq!(
            setup_name(Install::Rpm, "0.2.0-beta.1"),
            format!("UwULock-0.2.0-beta.1-linux-{arch}.rpm")
        );
        assert_eq!(
    ...
    fn the_names_installed_versions_look_for_stay() {
        ...
        assert_eq!(
            ...
            "UwULock-Setup-1.2.3.exe"
        );
        #[cfg(all(target_os = "macos", target_arch = "aarch64"))]
        assert_eq!(
            setup_name(Install::Setup, "1.2.3"),
            "UwULock-Setup-1.2.3-macos-arm64-update"
        );
        #[cfg(all(target_os = "macos", target_arch = "x86_64"))]
        assert_eq!(
            setup_name(Install::Setup, "1.2.3"),
            "UwULock-Setup-1.2.3-macos-x64-update"
        );
        #[cfg(all(target_os = "linux", target_arch = "x86_64"))]
        assert_eq!(
            setup_name(Install::Setup, "1.2.3"),
            "UwULock-Setup-1.2.3-linux-x64.AppImage"
        );
        assert!(setup_name(Install::Setup, "1.2.3").starts_with("UwULock-Setup-1.2.3"));
    }
