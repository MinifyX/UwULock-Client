...
export interface SetupApi {
  ...
  /** macOS and Linux: switch to uninstalling the installed UwULock. */
...
function previewApi(): SetupApi {
  ...
  const dir = 'C:\\Users\\Nyu\\AppData\\Local\\Programs\\UwULock';
  const pretend = async (steps: Step[]) => {
    ...
    if (params.has('fail')) throw `Couldn't write ${dir}\\UwULock.exe`;
  };
  return {
    info: async () => ({
    ...
    pickFolder: async () => 'D:\\Apps\\UwULock',
    closeApp: async () => {
      running = false;
    },
