...
//! [--wait-pid <pid>]` is how UwULock hands over to a downloaded update, and
...
struct Setup {
    layout: Layout,
    mode: Mode,
    /// Where the uninstaller removes UwULock from.
...
struct Info {
    ...
    has_payload: bool,
    sandbox: bool,
    /// Update mode: start UwULock again when done.
    relaunch: bool,
...
/// Lets the user pick a folder. On Windows and Linux UwULock goes into a
/// "UwULock" folder inside it; on macOS the app goes straight in, the way it
...
/// from, so the setup itself offers it when UwULock is there.
#[tauri::command]
fn begin_uninstall(setup: State<'_, Setup>) -> Result<(), String> {
    if cfg!(windows) {
        return Err("On Windows, UwULock is removed from Installed apps.".into());
    }
    let dir = setup
        .layout
        .installed()
        .map(|installed| PathBuf::from(installed.dir))
        .ok_or("UwULock isn't installed.")?;
...
pub fn run() {
    ...
    ) = (&mode, &uninstall_dir)
    {
        if let Ok(me) = std::env::current_exe() {
            let copy =
                std::env::temp_dir().join(format!("UwULock-Uninstall-{}.exe", std::process::id()));
    ...
    if !system::webview2_installed() {
        let (title, text) = if system_is_german() {
            (
                "UwULock Setup",
                "UwULock braucht Microsoft Edge WebView2. Soll es jetzt heruntergeladen und installiert werden?",
            )
        } else {
            (
                "UwULock Setup",
                "UwULock needs Microsoft Edge WebView2. Download and install it now?",
            )
        };
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // UwULock's. WKWebView keeps its data by bundle id and takes no folder.
            #[cfg(not(target_os = "macos"))]
            let window = window.data_directory(std::env::temp_dir().join("UwULock-Setup-WebView"));
            window.build()?;
            Ok(())
        })
        ...
        .expect("error while running UwULock Setup");
}
