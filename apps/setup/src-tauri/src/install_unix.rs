//! Installing, updating and removing UwULock on macOS and Linux, for the
//! current user.
//!
//! - **macOS:** `UwULock.app` into `/Applications`, or `~/Applications` when
...
//!   `~/.local/share/uwulock`, with menu entries in
...
//! UwULock's updater reads too: an app installed some other way — a `.deb`, a
//! copy dragged out of a disk image — is not one the setup updates.
//!
//! `UWULOCK_SETUP_SANDBOX=<folder>` redirects all of it for testing.
...
#[cfg(target_os = "macos")]
mod names {
    pub const APP: &str = "UwULock.app";
}
#[cfg(not(target_os = "macos"))]
mod names {
    pub const APP: &str = "UwULock";
}
use names::APP;

pub const APP_ID: &str = "app.uwulock.desktop";
const SETUP_ID: &str = "app.uwulock.setup";
...
pub struct Layout {
    ...
    #[cfg_attr(target_os = "macos", allow(dead_code))]
    desktop: PathBuf,
    /// Everything UwULock keeps: removed on request.
    data: Vec<PathBuf>,
    pub sandbox: bool,
}
...
impl Layout {
    ...
    fn real() -> Self {
        ...
        Self {
            default_dir: data_home.join("uwulock"),
...
pub fn folder_for(picked: PathBuf) -> PathBuf {
    if cfg!(target_os = "macos")
        || picked
            .file_name()
            .is_some_and(|name| name.to_string_lossy().eq_ignore_ascii_case("uwulock"))
    {
        picked
    } else {
        picked.join("uwulock")
...
/// Stops UwULock if it runs from `dir`. Skipped in the sandbox, which must
...
/// Removes a folder UwULock owns. A symlink in its place is only unlinked,
/// never followed.
...
pub fn install(
    ...
    let staging = dir.join(format!(".uwulock-setup-{}", std::process::id()));
    remove_tree(&staging);
...
fn shortcuts(layout: &Layout, dir: &Path, options: &Options) -> Result<(), String> {
    ...
    let entry = desktop_entry(
        &app_dir,
        "UwULock",
        "Password manager for Vaultwarden and Bitwarden",
        "uwulock-desktop",
...
mod tests {
    ...
    fn a_folder_the_person_picked_gets_a_folder_of_ours() {
        ...
        if cfg!(target_os = "macos") {
            ...
            assert_eq!(folder_for(picked), PathBuf::from("/opt/tools/uwulock"));
            assert_eq!(
                folder_for(PathBuf::from("/opt/uwulock")),
                PathBuf::from("/opt/uwulock")
            );
        }
    }
