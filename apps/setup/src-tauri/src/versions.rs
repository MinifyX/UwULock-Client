...
/// An update must never replace a newer UwULock with an older (validly signed)
...
pub fn check_not_older(installed: Option<&str>, update: &str) -> Result<(), String> {
    ...
    match (installed.and_then(parse), parse(update)) {
        (Some(installed), Some(update)) if update < installed => Err(format!(
            "UwULock {installed} is already installed. This update is older ({update}), so it was skipped."
        )),
        (Some(_), Some(_)) => Ok(()),
        _ => Err("It isn't clear which UwULock version is installed, so the update was skipped. Run the setup by hand instead.".into()),
    }
}
