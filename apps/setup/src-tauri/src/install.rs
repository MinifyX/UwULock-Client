//! Installing, updating and removing UwULock for the current Windows user.
//!
//! Everything lives under the user's profile: the app in
//! `%LOCALAPPDATA%\Programs\UwULock`, shortcuts in the Start menu (and on the
//! desktop if wanted), and registry entries under `HKEY_CURRENT_USER`. No
//! administrator rights are needed.
//!
//! `UWULOCK_SETUP_SANDBOX=<folder>` redirects all of it (files, shortcuts,
//! registry under `HKCU\Software\UwULock-Setup-Sandbox`) for testing.
...
pub const APP_EXE: &str = "UwULock.exe";
pub const UNINSTALL_EXE: &str = "uninstall.exe";
pub const APP_ID: &str = "app.uwulock.desktop";
const SHORTCUT: &str = "UwULock.lnk";
const HOMEPAGE: &str = "https://github.com/MinifyX/UwULock-Client";
const UNINSTALL_KEY: &str = r"Software\Microsoft\Windows\CurrentVersion\Uninstall\UwULock";
const SETUP_KEY: &str = r"Software\UwULock\Setup";
/// What Tauri's standard NSIS installer (0.0.1) called the app and where it
/// kept its own registry entry.
const LEGACY_EXE: &str = "uwulock-desktop.exe";
const LEGACY_PRODUCT_KEY: &str = r"Software\uwulock\UwULock";

static PAYLOAD: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/payload.zst"));
const PAYLOAD_SIZE: &str = env!("UWULOCK_SETUP_PAYLOAD_SIZE");
...
impl Layout {
    pub fn detect() -> Self {
        match std::env::var_os("UWULOCK_SETUP_SANDBOX").filter(|dir| !dir.is_empty()) {
            Some(dir) => Self::sandbox(Path::new(&dir)),
            None => {
                let folders = system::folders();
                Self {
                    default_dir: folders.user_programs.join("UwULock"),
                    ...
                    legacy_dir: folders.local.join("UwULock"),
    ...
    pub fn sandbox(root: &Path) -> Self {
        Self {
            default_dir: root.join(r"Programs\UwULock"),
            ...
            legacy_dir: root.join(r"Local\UwULock"),
            registry_prefix: format!(
                r"Software\UwULock-Setup-Sandbox\{}\",
                root.file_name()
...
fn extract(payload: &[u8], size: &str, target: &Path, progress: Progress) -> Result<(), String> {
    if payload.is_empty() {
        return Err("This setup was built without UwULock inside (a development build).".into());
    }
...
/// Stops UwULock if it runs from `dir`. Skipped in the sandbox, which must never
...
fn remove_legacy(layout: &Layout, new_dir: &Path) -> Result<(), String> {
    ...
    layout.remove_empty(r"Software\uwulock");
    Ok(())
}

/// The folder the setup installs into for a folder the person picked: a
/// "UwULock" folder inside it, unless it is one already.
pub fn folder_for(mut picked: PathBuf) -> PathBuf {
    if !picked
        .file_name()
        .is_some_and(|name| name.eq_ignore_ascii_case("UwULock"))
    {
        picked.push("UwULock");
    }
    picked
...
/// Starts the installed UwULock. Nothing in the sandbox.
...
pub fn install(
    ...
    let incoming = dir.join("UwULock.exe.new");
...
fn register(layout: &Layout, dir: &Path, options: &Options, version: &str) -> Result<(), String> {
    ...
    write(&entry, "DisplayName", "UwULock")?;
    write(&entry, "DisplayVersion", version)?;
    write(&entry, "Publisher", "UwULock")?;
...
pub fn uninstall(
    ...
    progress(Step::Register, 0.0);
    layout.remove_tree(UNINSTALL_KEY);
    layout.remove_tree(r"Software\UwULock");
    layout.remove_tree(LEGACY_PRODUCT_KEY);
    layout.remove_empty(r"Software\uwulock");
    progress(Step::Register, 1.0);
    ...
    for file in [
        ...
        if path.exists() && std::fs::remove_file(&path).is_err() && file == APP_EXE {
            return Err(format!(
                "Couldn't remove {}. Is UwULock still open?",
                path.display()
            ));
        }
    }
...
mod tests {
    ...
    fn remembers_defaults_until_something_is_installed() {
        ...
        assert!(options.dir.ends_with(r"Programs\UwULock"));
