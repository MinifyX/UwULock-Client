...
pub fn delete_after_exit(file: &Path) {
    ...
    let _ = std::process::Command::new(system.join("cmd.exe"))
        .raw_arg(
            "/c \"\"%UWULOCK_PING%\" 127.0.0.1 -n 4 > nul & del /f /q \"%UWULOCK_SETUP_COPY%\"\"",
        )
        .env("UWULOCK_PING", system.join("PING.EXE"))
        .env("UWULOCK_SETUP_COPY", file)
        .current_dir(&system)
...
pub fn install_webview2() -> Result<(), String> {
    ...
    let folder = std::env::temp_dir().join(format!("UwULock-WebView2-{}", std::process::id()));
