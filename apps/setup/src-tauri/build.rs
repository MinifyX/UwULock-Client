//! Packs UwULock into the setup executable.
//!
//! On Windows `UWULOCK_SETUP_PAYLOAD` points at the built `uwulock-desktop.exe`,
//! compressed on its own.
//!
//! On macOS and Linux the app is a folder — `UwULock.app`, or the AppDir
//! `UwULock` — and `UWULOCK_SETUP_PAYLOAD` points at a folder holding it. It goes
...
fn pack_folder(variable: &str) {
    ...
    println!("cargo:rustc-env=UWULOCK_SETUP_PAYLOAD_SIZE={size}");
}

fn main() {
    if std::env::var("CARGO_CFG_TARGET_FAMILY").as_deref() == Ok("unix") {
        pack_folder("UWULOCK_SETUP_PAYLOAD");
    } else {
        pack_file(
            "UWULOCK_SETUP_PAYLOAD",
            "payload.zst",
            "UWULOCK_SETUP_PAYLOAD_SIZE",
        );
    }
